...
# Google Sheets Service Key and Sheet ID
...
def start(update, context) -> None:
    text0 = (
        "Hello and welcome. I'm Botohyao. Looking for places to eat?"
    )
    text = (
        ...
        "To cancel, select 'Exit' button or type /stop."
    )
...
def randomSample(update, context):
    ...
    filtered = list(filter(filterEverything, master_lst))
...
def getBbtSample(update, context):
    ...
    filtered = list(filter(filterNearby, bbt_lst))
    sample = random.sample(filtered, min(len(filtered), 5))
    if len(sample) == 0:  # if there's no entries found
...
def stop_nested(update, context):
    ...
    logger.info("User [%s] canceled conversation within nested using command [/stop].",
                update.message.from_user.first_name)
    return STOPPING
...
def moreinfo(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text='\n\nThis is a private list of food places by my girlfriend and myself we wish to visit. '
                                                                    'I created this bot to serve as an assistant to retrieve/filter data easily '
...
def main():
    ...
    global master_lst
    global bbt_lst
    master_lst = worksheet.get_all_records(empty2zero=True)
